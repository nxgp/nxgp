...
import { loopVisuals } from './mockups/LoopVisuals'
...
export function OperatingModel() {
  ...
  const Visual = loopVisuals[active]

  if (stacked) {
    return (
      <section id="how-we-work" className="section">
        <div className="shell">
          <Eyebrow>{operatingModel.kicker}</Eyebrow>
          <h2 className="t-h2 mt-4 max-w-[18ch]">{operatingModel.h2}</h2>
          <p className="t-lead mt-5 max-w-[40rem]">{operatingModel.sub}</p>
          <div className="mt-12 flex flex-col gap-12">
            {operatingModel.steps.map((s, i) => {
              const V = loopVisuals[i]
              return (
                <div key={s.title} className="grid gap-6 sm:grid-cols-2 sm:items-center">
                  <div>
                    <span className="text-[0.8rem] font-800 text-accent-deep">{s.n}</span>
  ...
  return (
    <section id="how-we-work" ref={section} className="relative">
      <div ref={pinned} className="flex min-h-screen items-center overflow-hidden">
        <div className="shell grid w-full items-center gap-10 py-16 lg:grid-cols-[1fr_1.05fr] lg:gap-16">
          <div>
            <Eyebrow>{operatingModel.kicker}</Eyebrow>
            <h2 className="t-h2 mt-4 max-w-[16ch]">{operatingModel.h2}</h2>
            <p className="t-lead mt-5 max-w-[38rem]">{operatingModel.sub}</p>
              ...
              {operatingModel.steps.map((s, i) => {
                ...
                return (
                  <button
                    key={s.title}
                    onClick={() => setActive(i)}
                    className={cn('group relative rounded-inner border px-5 py-4 text-left transition-all duration-500', on ? 'border-line bg-surface shadow-soft' : 'border-transparent bg-transparent opacity-55 hover:opacity-80')}
                  >
                    ...
                    <span className="flex items-center gap-3">
                      <span className={cn('flex size-7 items-center justify-center rounded-full text-[0.74rem] font-800 transition-colors duration-500', on ? 'bg-accent text-white' : 'bg-ink/5 text-ink-soft')}>{s.n}</span>
