 ...
 * business outcomes on the right. Each animatable element carries `data-console`
          ...
          <ul className="flex flex-col gap-2">
            {feed.map((f) => (
              <li key={f.text} data-console className="flex items-center gap-3 rounded-inner border border-line bg-surface px-3 py-2.5">
                ...
                {f.tone === 'done' && <span className="ml-auto flex items-center gap-1 rounded-pill bg-accent-wash px-2 py-0.5 text-[0.66rem] font-700 text-accent-deep"><Check className="size-3" /> live</span>}
                {f.tone === 'win' && <span className="ml-auto rounded-pill bg-accent-wash px-2 py-0.5 text-[0.66rem] font-700 text-accent-deep">outcome</span>}
                {f.tone === 'run' && <span className="ml-auto rounded-pill bg-bg px-2 py-0.5 text-[0.66rem] font-700 text-ink-soft">queued</span>}
              </li>
            ))}
            ...
            <li data-console className="mt-1 flex items-center gap-3 rounded-inner bg-ink px-3.5 py-3 text-white">
              ...
              <span className="text-[0.82rem] font-700">Tied to a business outcome</span>
              <span className="ml-auto text-[0.78rem] font-800 text-au-mint">+18% pipeline</span>
            </li>
          </ul>
        </main>
      </div>
